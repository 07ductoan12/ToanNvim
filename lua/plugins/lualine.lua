return {
	...
	-- Statusline plugin with many customizations.
	{
		...
		opts = function()
			...
			local ToanUtil = require('util.init')
			...
			local active = {
				bg = ToanUtil.ui.bg('StatusLine'),
				fg = ToanUtil.ui.fg('StatusLine'),
			}
			local inactive = {
				bg = ToanUtil.ui.bg('StatusLineNC'),
				fg = ToanUtil.ui.fg('StatusLineNC'),
			}
			...
			local theme = {
				normal = {
					...
					c = active,
					x = {
						fg = ToanUtil.color.brightness_modifier(active.bg, -80),
						bg = active.bg,
					},
					y = {
						fg = active.fg,
						bg = ToanUtil.color.brightness_modifier(active.bg, -20),
					},
					z = {
						fg = active.fg,
						bg = ToanUtil.color.brightness_modifier(active.bg, 63),
					},
				},
				inactive = {
					a = inactive,
				...
				inactive_sections = {
					lualine_a = {
						...
						{ Util.lualine.pretty_path(), padding = { left = 1, right = 0 } },
						{
							...
							cond = is_file_window,
							padding = 1,
							color = { fg = ToanUtil.ui.bg('DiffDelete') },
						},
					},
					lualine_b = {},
					lualine_c = {},
